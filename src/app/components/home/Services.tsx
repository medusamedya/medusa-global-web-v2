...
export default function Services() {
  ...
  return (
    <section className="relative w-full py-24 bg-background transition-colors duration-500 overflow-hidden">
      ...
      {/* İnce grid deseni - Teknolojik derinlik hissi */}
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col items-center justify-center w-full relative z-20">
          <div className="bg-background p-2 rounded-full border border-foreground/10 dark:border-white/10 shadow-sm relative overflow-hidden group">
        ...
        {/* Sadece 4 kolonlu düzende (lg) görünecek, -mt-4 ile mouse ikonunun hemen altından başlar */}
        <div className="hidden lg:block relative w-full h-32 mb-6 opacity-80 -mt-2 z-0 pointer-events-none">
          {/* viewBox kullanımı ile yüzdelik dilim hatası çözüldü */}
          <svg className="w-full h-full drop-shadow-md overflow-visible" viewBox="0 0 1000 100" preserveAspectRatio="none" fill="none">
            ...
            {servicesData.map((service, index) => {
              // 1000 px baz alınarak 4 kolonun tam orta noktaları
              const targetX = [125, 375, 625, 875][index];
              const isHovered = hoveredId === service.id;

              return (
                <g key={`path-${service.id}`}>
                  {/* Sabit Duran Kılavuz Çizgi (Referans görseldeki yapı) */}
                  <path 
                  ...
                  {/* HOVER DURUMUNDA: Karta doğru akan veri akışı */}
                  {isHovered && (
                    <path 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 relative z-10 lg:-mt-4">
          {servicesData.map((service) => (
            ...
            >
              {/* Premium Animasyonlu İkon Çerçevesi */}
              ...
              <div className="space-y-3 px-2">
                <h3 className="text-xl font-bold text-foreground dark:text-white tracking-wide transition-colors duration-300 group-hover:text-medusa-gold">
                  {service.title}
                </h3>
                <p className="text-foreground/80 dark:text-white/80 text-sm leading-relaxed max-w-[250px] mx-auto transition-colors duration-300">
                  {service.description}
                </p>
              </div>
            </div>
